...
### CRITICAL: never change `url` in CrawlingSites.ts to fix a domain/URL change

Changing `url:` in `CrawlingSites.ts` changes `this.baseUrl` which feeds the `url` field on `AdVehicle`, which feeds `storeId = md5(url)`. A different storeId = a **different S3 key** = a new vehicle object in S3 and ES instead of updating the existing one. Old vehicles accumulate as orphans and eventually get deactivated. This is wrong.

**Symptom:** the broken site is alive again, vehicle count looks normal, but ES now has two copies of every vehicle — one with the old domain (gradually deactivating) and one with the new domain (freshly created). S3 bucket doubles in size.

When a site's URL/domain changes, use the **workingUrl fix pattern** instead:

### How to implement a workingUrl fix

Use when: the site's detail URL format changed but you can still compute both the old (legacy) and new (working) URL in code. Examples: domain `oxylio.com` → `www.oxylio.com`; path `/vehicles/car/{bodyType}/{id}` → `/en/vehicle/{id}`.

**For small sites / `shouldValidateListingVehicle: false`** (all three steps at once):

1. **Override `fetchRequest()`** to redirect actual HTTP requests to the new URL while keeping the `url` field (legacyUrl) unchanged:
```ts
protected async fetchRequest<T>(url: string, options?: FetchRequestOptions): Promise<T> {
    return super.fetchRequest(url.replace('https://old-domain.com', 'https://new-domain.com'), options);
}
```

2. **In `parseVehicleInput()`**, set `workingUrl` to the new-domain URL and keep `url` as the legacy URL:
```ts
url: parseVehicleParams.url,           // legacy - used for storeId
workingUrl: parseVehicleParams.url.replace('https://old-domain.com', 'https://new-domain.com'),
```
Or if the crawler builds a dedicated `buildWorkingUrl()` helper, hardcode the new domain there.

3. **In `getBrandsAndModels()` (or listing parsing)**, if a partial vehicle or listingUrl is assigned, also set `workingUrl` on the partial vehicle so SVL gets it.

**For large sites / `shouldValidateListingVehicle: true`** — implement steps 1+2 first (details only), let `workingUrl` propagate gradually over days/weeks, then add step 3. Adding step 3 early triggers SVL failures for all vehicles that don't yet have `workingUrl`, causing a massive detail-crawl spike. See [Working URL fix docs](https://preskok.atlassian.net/wiki/spaces/M/pages/3002302476/Working+URL+fix) for the full sequencing.

**CrawlingSites.ts `url:` stays unchanged** throughout — it is the legacyUrl anchor, not the fetch target.
...
## Turnover calculation caveat (each ad = unique vehicle)

Turnover (DCM stock metrics) is calculated per-ad: each distinct ad/URL is treated as a unique vehicle, and turnover is the number of days that specific ad stayed active. Dealers who delete and re-publish the same physical vehicle as a new ad (common pattern) fragment its real lifetime into several short-lived "vehicles" — e.g. one vehicle re-listed 15 times over 30 days shows as 15 vehicles with ~2-day turnover each, when the real turnover for that physical car is 30 days. Keep this in mind before treating a turnover anomaly as a crawler bug — it can be a genuine artifact of how dealers manage listings on a given site, not a data-quality issue.
...
**Separate manual spot-check tool** (`npm run test-raw`, `test-raw.mjs`): set `esstage`/`esprod` vars to your ES credentials, pick a site in the query for getting random storeIds, run `npm run test-raw`. Reports land in `/cache/<siteName>/<storeIdValue>/s3_report.txt` and `es_report.txt` for each of the 20 sampled storeIds — raw field values should match the non-raw (mapped) fields shown alongside them. Useful for confirming a mapping change didn't regress a site before/after deploy, independent of the automated Marko's-script email alerts above.
