...
### 21b. Site's own pagination is broken (not our URL construction)
**Signal:** Per-listing/per-model vehicle count is suspiciously flat at a round number (e.g. always exactly one page's worth — 12, 20, 24) regardless of how many the site itself claims are available, with NO errors in our logs — because page 1 always succeeds and we simply never reach page 2.
**Diagnosis:** Follow the site's own "next page" link (`getNextPageUrl()` result) directly in a browser, independent of our crawler. Two site-side variants seen:
  1. Mistemplated static link — href truncates/mangles the model slug (e.g. `peugeot-208` → `p-2`, `peugeot-5008` → `p-5`) and 404s on the site's own domain.
  2. Real backend broken — the site's actual infinite-scroll/pagination endpoint (found via network tab while manually scrolling, e.g. a `POST`-only JSON endpoint or a `/list/page-N?...` URL) itself 500s or silently no-ops, independent of any URL we guess.
**Example:** e-motors, 2026-07-15 — Peugeot 5008 states "215 véhicules" on-site, page 1 caps at 12, `.next-cars` href resolves to a mistemplated 404, and the site's real infinite-scroll endpoint (`/liste-de-nos-vehicules/page-2?marque[]=peugeot&order-by[]=dispo_asc`) 500s when hit directly — confirmed broken for real site visitors too, not just us.
**Fix:** None available on our side if the site's own mechanism is broken — don't spend time guessing alternate URL patterns once you've confirmed via direct browser navigation that the site's own link/endpoint fails. Document as a site-side limitation with an evidence trail (screenshot/network log) and move on. Re-check periodically in case the site fixes their own infra.
**Don't mistake rising vehicle counts for the bug resolving itself** — under this pattern, total count can still climb day over day even while page-2+ stays permanently broken. Check `VehicleListUrl` (or equivalent per-model listing URL) cardinality per day (`date_histogram` on `CreatedAt`/first-seen + `cardinality` agg) — if the number of *distinct model pages* discovered is growing (`getBrandsAndModels()` catching up / retrying previously-failed model pages), that alone explains new-vehicles-per-day ≈ new-models-per-day × page-cap, with the per-model depth (page 1 only) never actually improving. E-motors 2026-07-16: +83 newly-discovered model pages × ~12/page ≈ 996 new vehicles, matching an observed "+1000 vehicles" day exactly — pagination was still 404/500-broken.
**Tags:** `pagination-broken` `url-change` `false-alarm` `drop-gradual`
...
**Fix:** `AWS_PROFILE=<prod-profile> aws s3 rm s3://$AWS_S3_BUCKET_DAILY_CACHE/YYYYMMDD/[hash]`. Deploy `isServerError()` improvement first to prevent re-save.
...
### 33c. Site serves alt template variant → real 200, correct content, one widget missing → mis-cached
**Signal:** `getBrandsAndModels` (or any facet/selector-dependent method) returns 0. Cached S3 body has a genuinely legitimate `statusCode:200`, correct title, correct category/content, real listing items present — but the ONE selector the parser needs matches 0 elements. Not truncated, not empty, not an anti-bot page.
**Root cause:** The site occasionally serves a different page template for the exact same URL/request — e.g. an SEO/lite variant that swaps a structured widget (brand-facet nav, filter block) for filler text (an SEO description paragraph) in that same DOM position. Confirm by diffing the cached body against a known-good live fetch of the same URL, anchored on a stable landmark just before the missing widget (e.g. breadcrumb `</ul>`) — in the bad copy, the widget's shimmer placeholder AND its real content div are both entirely absent from the DOM, not just unfilled.
**Example:** pazar3 2026-07-16 — `.groups-count a` (brand facet) selector matched 0 on the cached `/ads/vehicles/automobiles/for-sale` response; breadcrumb was immediately followed by `<div class="shortDescription visually-hidden">` instead of the `groups-count-shimmer` → `groups-count` sequence seen on a fresh fetch. Same signature as the 2026-07-02→07-03 pazar3 incident (see `sites/pazar3.md`).
**Fix:** Operational only — delete the poisoned S3 key (prod: `AWS_PROFILE=<prod-profile> aws s3 rm ...`, read-only `AMS_S3_PROD_*` creds will correctly refuse) + rerun. No code fix — the selector is correct; it just doesn't exist in this one substituted template.
**Generalizable:** Any site built on a template/rendering pool that can non-deterministically pick an alternate layout for the same route (A/B test, load-balanced template variants, bot-heuristic-triggered lite mode). Before assuming a selector broke from a site redesign, confirm whether it's actually a *rotating* template by re-fetching live — if a fresh fetch has the widget back, it's this pattern, not a permanent site change.
**Tags:** `fake-200` `s3` `selector` `getBrandsAndModels` `prepared-0`
...
### 38b. Deactivation-lock recovery leaves inflated active count
**Signal:** After fixing a multi-day 0-vehicle/broken-crawl regression, ES active count for the site looks suspiciously high/unchanged despite the outage — because the site was under a deactivation lock (`~foundational.md` "Deactivation safeguard") for the duration, so nothing deactivated while the crawler found 0 new vehicles. Query `vehicle_visit` (`SELECT site, todayCount, prevCount, ratioPercent`) — a low `ratioPercent` (e.g. ~50%) means only that fraction of yesterday's active set was re-confirmed by today's crawl; the rest are stale docs that will deactivate the instant the lock is lifted.
**Example:** e-motors, 2026-07-15 — locked during the 07-09/07-13 URL-restructure regression; ~2,832 "active" but `ratioPercent` 49.51% (1,551 vs 1,521) meant ~50% were actually gone.
**Fix:** Do NOT just remove the lock — a ~50% single-night drop will likely re-trip the mass-deactivation safeguard and re-lock the site, creating a lock/unlock cycle. Manually deactivate the stale portion first (batch, outside the automated nightly run), then lift the lock.
**Tags:** `deactivation-lock` `stale` `mass-deactivation` `false-alarm` `drop-gradual`
...
**Example:** willhaben IP `<whitelisted-ip>` revoked Mar 2025.
...
**Diagnostic:** Compare per-day Graylog/ES metrics across the affected window. Look for: (1) volume spike on day N, (2) success rate inversion on day N, (3) `forbidden` count higher on N+1, N+2 even though volume is back to normal.
