...
URL: `.env` → `GRAYLOG_API_URL` / `GRAYLOG_AUTH_TOKEN` (active by default = local, Graylog 4.2 on devenv). Stage/prod endpoint and credential resolution is intentionally not spelled out here — resolve it from session context, or ask the user.
...
```bash
AUTH="${TOKEN}:token"   # build separately - inlining "$TOKEN:token" directly into -u can mangle the colon in some shells and triggers an interactive password prompt
...
For multi-day per-metric matrices, run the same query per day over the affected window and diff the results.
...
Quick health snapshot: if `Response was forbidden` count is rising day-over-day while `Finished crawling listing url` stays stable, anti-bot is adapting — **compare per-day Graylog/ES metrics across the affected window** to confirm.

---

## Managing inputs via the REST API (local devenv)

Endpoints for listing/deleting/restarting a GELF/Syslog input directly (management UI equivalent):

```bash
AUTH="${TOKEN}:token"
# list all inputs with their bound port
curl -s -u "$AUTH" "$GRAYLOG_API_URL/api/system/inputs" -H "X-Requested-By: curl" | jq '.inputs[] | {id, title, type, port: .attributes.port}'

# delete an input - note the path is plural "inputs", NOT "input"
curl -s -X DELETE -u "$AUTH" "$GRAYLOG_API_URL/api/system/inputs/<INPUT_ID>" -H "X-Requested-By: curl"

# restart/start a stopped or misfired input - different resource entirely: "inputstates", not "inputs/.../restart"
curl -s -X PUT -u "$AUTH" "$GRAYLOG_API_URL/api/system/inputstates/<INPUT_ID>" -H "X-Requested-By: curl"
```

`DELETE /api/system/input/<id>` (singular) and `PUT /api/system/inputs/<id>/restart` both 404 - these are not real Graylog API routes.

**Diagnostic:** if the local devenv logger stops receiving anything at all (Graylog itself is up, container healthy) despite the app clearly sending GELF messages, check `docker logs <graylog-container>` for `java.net.BindException: Address already in use` on `[GELF UDP]` startup - a second, unrelated input (e.g. a stray manually-created "Postman test" input) can be bound to the same port (`GRAYLOG_PORT` in `.env`, typically `12201`) and wins the race on container start, leaving the real input `FAILED`. Delete the duplicate, then restart the real input via `inputstates` above.
