...
These rules are applied during `the crawl-health check` runs after reading the Baselines section of the history canvas.
...
## Baseline computation (used by `the crawl-health check sync mode` and the dedup script)
