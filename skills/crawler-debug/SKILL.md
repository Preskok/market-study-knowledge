...
- **Gradual multi-day drop with steady requests** → anti-bot pressure, NOT a code bug. Switch to a multi-day trend comparison — pull per-day metrics from Graylog + ES side-by-side and check if anti-bot is adapting, proxy reputation is burnt, or it's noise.
...
1. Delete S3: invoke `the S3 fetch tool stage <url> --delete --date=YYYYMMDD` (or `--body='{...}'` if it was an API request). Prod is intentionally read-only via the S3 fetch tool — for prod deletes use the AWS CLI directly: `AWS_PROFILE=<prod-profile> aws s3 rm s3://$AWS_S3_BUCKET_DAILY_CACHE/[YYYYMMDD]/[hash]`. For inspecting the cached response before deciding to delete, use `the S3 fetch tool <url>` (READ).
...
- Multi-day gradual drop → compare per-day Graylog/ES metrics across the affected window instead.
...
- **No sensitive literals in saved files** — never write actual bucket names, account IDs, or AWS profile values into reference/site docs. Always use env var placeholders: `$AWS_S3_BUCKET_DAILY_CACHE`, `$AWS_PROFILE`. Correct form: `AWS_PROFILE=<prod-profile> aws s3 cp s3://$AWS_S3_BUCKET_DAILY_CACHE/YYYYMMDD/<hash> - | head -c 500`.
