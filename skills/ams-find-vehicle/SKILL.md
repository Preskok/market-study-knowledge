...
description: Use when a vehicle is missing from search results, expected to be crawled but not showing up in ES, or when debugging "where did this vehicle go" across the pipeline (ES indices, S3, Graylog). Also use when asked to trace a specific vehicle by URL or storeId on stage/local.
---

# ams-find-vehicle

**Prod is explicitly forbidden here — do not use prod envs or run this against prod data.**
This skill is restricted to `stage`/`local` only.
...
| Env (stage / local — never prod) | Determines which ES + S3 to hit |
...
`storeId` does NOT exist in this index — search by `URL` only. Check `_mapping/field/URL` first: if it's already `type: keyword`, query the bare field name; only append `.keyword` if the mapping shows `type: text`.
```bash
# By legacyUrl (stored as-is, uppercase URL field — try without .keyword first)
curl -s "$ELASTIC_SEARCH_URL/$ELASTIC_SEARCH_INDEX/_search" \
  -H "Content-Type: application/json" \
  -d '{"query": {"term": {"URL": "LEGACY_URL_HERE"}}, "size": 1, "_source": ["URL","Site","storeId","isActive"]}'

# By exact doc _id (when you already have it, e.g. from Kibana) — alias spans multiple indices, use ids query not GET _doc/<id>
curl -s "$ELASTIC_SEARCH_URL/$ELASTIC_SEARCH_INDEX/_search" \
  -H "Content-Type: application/json" \
  -d '{"query": {"ids": {"values": ["DOC_ID_HERE"]}}}'
```
...
|--------|-----------|--------------|
| Stored vehicle JSON | Normalised AdVehicle; source of truth for ES write | `the S3 fetch tool STOREID [env]` |
| Raw response cache | Daily HTML/JSON from the site | `the S3 fetch tool URL [env]` (keyed `YYYYMMDD/md5(url)`) |

```bash
# Stored vehicle JSON (most useful — confirms crawler saved it)
# Run: the S3 fetch tool <storeId> [stage|local]

# Raw response cache (confirms crawler fetched the page today)
# Run: the S3 fetch tool <vehicle-url> [stage|local]
```
...
# the S3 fetch tool STOREID [stage|local]
...
- **legacyUrl ≠ workingUrl in ES.** Old search stores `legacyUrl` in `URL`. Data index stores `workingUrl` in `url`. Always try storeId, not URL, for cross-index lookups **in the data index**.
- **`storeId` does not exist in the old search index at all** (confirmed via `_mapping/field/storeId` — empty mapping across every backing index). It's data-index-only, computed downstream in the mapping pipeline and never written to the old index. In the old index, look up by `URL` (or `_id` if you already have the raw ES doc id) — do NOT assume `storeId` works there, it will silently return 0 hits.
- **Field mapping varies by environment/index — always check `_mapping/field/<field>` before trusting a `.keyword` suffix.** On local devenv, `URL`/`FuelType`/etc. in the old index are mapped as plain `keyword` (not `text` with a `.keyword` subfield) — querying `URL.keyword` returns 0 hits silently instead of erroring, easy to mistake for "not found." Query the bare field name (`URL`, not `URL.keyword`) when the mapping says `type: keyword` already. This applies to both indices in local devenv; verify per-environment since stage may map differently.
- **Fetching a doc by `_id` on a multi-index alias** (rollover aliases always are) — `GET <alias>/_doc/<id>` errors with `"alias has more than one index associated with it"`. Use `POST <alias>/_search {"query":{"ids":{"values":["<id>"]}}}` instead.
