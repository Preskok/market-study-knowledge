...
- **(undated, ~2023)** — Listings show "promoted" ads twice: once tagged `VETRINA` (promoted slot) on one listing page, then again as a normal (non-promoted) entry on another page. The listing-page iteration already dedups by details URL before publishing to queue, so this doesn't create duplicate vehicles — noted here in case a future listing-parsing change breaks that assumption.
