...
- **2026-05-08** — local testing requires VPN: `fetchRequest` hardcodes `useProxy=true` with `PRESKOK_SET_2` (<office-proxy-host>:9001/9005). Direct curls to `autoconnect.al/api/data/makes` work without proxy, so the proxy is optional for the site itself but mandatory because of the crawler config.
