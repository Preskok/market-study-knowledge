...
Prod S3 is read-only via `the S3 fetch tool` by design. Deleting a prod cache entry requires prod write access — ask the user to do it, or get explicit confirmation before Claude does. The exact access mechanism is intentionally not documented here.
...
After parser fix deployed. Messages in `MS_DL`, `MS_BULK_SAVE_DL`, `MS_BULK_DL`, `MS_TASKS_DL`. Requires RMQ management access — ask the user to do it, or get explicit confirmation before Claude does. The exact access mechanism is intentionally not documented here.
...
For stuck message (infinite loop), restart consumer. Purging requires RMQ management access — ask the user to do it, or get explicit confirmation before Claude does.
...
1. Connect to the relevant Redis instance — for prod, this requires connection info Claude doesn't hold in this file; ask the user, or get explicit confirmation before Claude proceeds. Local/stage connection details are in `.env`.
2. Find the key (naming usually per-site or service-level like `datadomeService`).
3. Delete it. Next request regenerates.
...
AWS_PROFILE=<prod-profile> aws s3 cp \
...
### Post-redesign URL scheme — follow the site's own link, don't pattern-match a fragment
When a site restructures its URLs and a crawler's guessed replacement pattern still 404s, don't keep guessing variants of the same pattern. Instead, find a page that shows the target content through the site's own real navigation (a category/summary page, a "view all" or "see versions" button) and read the literal `href` it resolves to — that's the actual working pattern, not an assumption. Example: e-motors (2026-07-14) — after the site dropped its combined per-brand page, the natural guess was to insert the condition segment into the model URL too (`/mandataire-{brand}-{condition}-{model}`), which 404s. The real pattern, found by clicking the site's own "Voir les versions" link from its model-summary page, was `/mandataire-{brand}-{model}` with no condition segment at all — condition only mattered for a different, unrelated intermediate page. A single broken example doesn't reveal enough of the taxonomy to safely generalize from.
