...
|---|---|---|---|
| 2 | `curl {url}` | free | — |
| 3 | `curl -x http://<office-proxy-host>:8000 {url}` | free | office network / VPN |
...
| 7 | Puppeteer + proxy (`--proxy-server=http://<office-proxy-host>:8000`) | free | office network / VPN |
...
- Proxy: `<office-proxy-host>:8000` (office network / VPN)
