...
- **No sensitive literals** — never write actual bucket names, account IDs, or profile names into saved files. Use env var placeholders: `$AWS_S3_BUCKET_DAILY_CACHE`, `$AWS_PROFILE`, etc. Example correct form: `AWS_PROFILE=<prod-profile> aws s3 cp s3://$AWS_S3_BUCKET_DAILY_CACHE/YYYYMMDD/<hash> - | head -c 500`.
