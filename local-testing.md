...
Keep `AWS_S3_BUCKET_DAILY_CACHE_PERMISSION_WRITE=true` in `.env` by default — this is the correct local default, not a flag to disable. With WRITE off, nothing crawled locally is ever cached, so every repeated local test run re-fetches live HTTP (slow, burns proxy/ScrapeDo credits). Only when you specifically need a fresh (non-cached) response, delete just that cache key instead of turning WRITE off:
- Per-site/all: `test/requests/Crawler/Crawler.http` (the `delete-daily-cache` request) or curl with your token from `http-client.env.json`. Wipes ALL sites' cache, not per-site.
- Per-key: `the S3 fetch tool local <storeId> --delete`.

**Never combine `WRITE=true` with `.env` pointed at prod or stage S3 credentials/buckets** — that would write local test traffic into the shared prod/stage cache. Local write must only ever target the local bucket.
...
mysql -h mysql.devenv -u root -p<devenv-password> --skip-ssl marketstudy -e "TRUNCATE TABLE vehicle_visit;"

# Import CSV — handles nullable runOnNthDays
mysql -h mysql.devenv -u root -p<devenv-password> --skip-ssl marketstudy -e "
...
# Verify
mysql -h mysql.devenv -u root -p<devenv-password> --skip-ssl marketstudy -e "SELECT COUNT(*) FROM vehicle_visit;"
```
